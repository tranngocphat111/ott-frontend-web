...
const OtpStep: React.FC<OtpStepProps> = ({
  targetEmail, otp, onChange, onSubmit, onResend, isLoading,
}) => {
    ...
    <form onSubmit={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: otp.join('').length === 6 && !isLoading ? 'var(--shadow-md)' : 'none',
      ...
      {/* Resend */}
      {onResend && (
        <div style={{ textAlign: 'center', borderTop: '1px solid var(--color-primary-100)', paddingTop: 14 }}>
          <button
            type="button"
            onClick={onResend}
            disabled={isLoading}
