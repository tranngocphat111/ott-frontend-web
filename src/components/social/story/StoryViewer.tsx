...
const StoryViewer: React.FC<Props> = ({
  ...
  const handleLike = async (key: ReactionKey) => {
    ...
    setReactionMap((prev) => {
      const next = { ...prev };
      if (previousReaction) {
        next[previousReaction.toLowerCase()] = Math.max(
          0,
          (next[previousReaction.toLowerCase()] || 1) - 1,
        );
      }
      if (!isRemoving) {
            ...
            <div className="absolute top-6 left-5 right-5 z-20 flex items-center justify-between text-white">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  {isSaved ?
                    <BookmarkCheck className="size-4 text-primary-400" />
                      ...
                      <div
                        ...
                        : item.type === "TEXT" && item.textContent ?
                          <div
                            ...
                            style={{
                              backgroundColor:
                                item.textBackgroundColor || "rgba(0,0,0,0.5)",
                            }}>
