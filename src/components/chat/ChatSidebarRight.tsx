...
const ChatSidebarRight: React.FC<ChatSidebarRightProps> = ({
  ...
  const { conversations, updateConversation, updateParticipant } = useConversations();
  ...
  const filterValidMembers = (members: any[]): ConversationMember[] => {
    ...
    return members
      .filter((member) => member && typeof member === "object" && member.user_id)
      .map((member) => ({
        _id: member._id,
  ...
  const loadSidebarData = useCallback(async () => {
    ...
    try {
      ...
      const [membersResult, pinnedResult, mediaResult, filesResult, linksResult] = results;
      ...
      const filesData = filesResult.status === 'fulfilled' ? filesResult.value : [];
  ...
  const handleUnpinMessage = async (messageId: string) => {
    try {
      ...
      await MessageService.pinMessage(conversation._id, messageId, currentUser._id, false);
  ...
  const handleRoleUpdated = async (userId: string, role: string) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Không thể cập nhật vai trò");
    }
  };

  const handleMediaClick = useCallback((messageId: string, imageIndex: number = 0) => {
      ...
      <div className="fixed right-0 top-0 h-full w-80 bg-white border-l border-gray-200 shadow-lg z-40 overflow-y-auto">
        ...
        {viewMode === "main" && (
          <>
            ...
            {/* Members Section - Navigation only, no expand */}
            {isGroupChat && (
              <CollapsibleSection
            ...
            <CollapsibleSection
              ...
              defaultOpen={true}
            >
              <FilesList messages={fileMessagesPreview} onViewAll={handleViewAllFiles} />
            </CollapsibleSection>
